...
def decoder_for_gpt3(args, input, role_selection=False):
    ...
    while attempt <= max_attempts and not success:
        try:
            if engine == "gpt-3.5-turbo":
                ...
                elif args.method == "adaptive_role_play":
                    if role_selection:
          ...
          #or openai.error.RateLimitError or openai.error.APIError or openai.error.ServiceUnavailableError:
            print(e)
            attempt += 1
...
class Decoder():
    ...
    def decode(self, args, input, role_selection = False):
        response = decoder_for_gpt3(args, input, role_selection)
        return response
    ...
    if len(pred) == 0:
        ...
        if args.method in ("few_shot", "few_shot_cot"):
            if answer_flag:
                ...
                pred = pred[-1]
        elif args.method in ("zero_shot", "role_play", "adaptive_role_play"):
            # choose the first element in list ...
            pred = pred[0]
        else:
